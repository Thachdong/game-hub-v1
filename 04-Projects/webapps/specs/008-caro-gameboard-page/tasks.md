...
- [X] T056 [P] Run `turbo run lint --filter=web` (package is named `web`, not `@game-hub/web`) — clean, no findings, across every new/modified file
- [X] T057 [P] Run `turbo run test --filter=web` and confirm the full suite is green — 196 tests passing across 49 files; `tsc --noEmit` also clean
- [X] T058 Executed quickstart.md's manual/live validation against a real running pair: started `04-Projects/api` (Postgres already up via its `docker-compose.yml`) and `apps/web` (`next dev`), minted two accounts directly in `account_social.accounts` plus JWTs signed with the dev `JWT_ACCESS_SECRET` (no password-based auth endpoint exists — Google OAuth only), and drove a real match through every state via curl + the live gameboard page: guest view of state 1 (board, own-creator-card fallback, waiting placeholder, chat), state 2 after a real join (both cards, live countdown), auto-cancellation when the 15s window elapsed (confirms FR-006 against the real timer), a fresh match started and immediately surrendered to reach state 4 (Review Moves control present). All test data (2 accounts, 4 matches, 1 game config) cleaned up afterward; pre-existing fixture rows were left untouched. **Found and fixed a real bug this task exists to catch**: `MatchState.playerX`/`playerO` are both `null` until a second player joins (only populated by `JoinMatchUseCase`), so state 1's own-player-card requirement (FR-003) rendered nothing at all against the real backend. Not listed in research.md's known-gaps table. Fixed in `GameboardSidePanel` with a `resolveState1OwnPlayer()` fallback: the session's own username when the current viewer is the creator, otherwise `creatorId` (reusing `PlayerCard`'s existing username-equals-id → `Player {id.slice(0,6)}` fallback). The tight 15s move-timer made a live in-progress move-placement check impractical over several sequential tool round-trips (two attempts both auto-completed via move-timeout before the request landed) — that path stayed covered by `GameboardContainer.test.tsx`'s mocked-event tests instead.
- [X] T059 Verified: `StartCountdown` renders the non-creator's Start control visibly disabled, not hidden (FR-004/CHK003/CHK007/CHK020 — see `components/molecules/StartCountdown.tsx`'s `isParticipant && !isCreator` branch); `ViewerListItem`'s mute/kick control maps to the single `muteMatchViewerAction`, matching FR-016/CHK021's "kick and mute are the same action" conclusion — confirmed against the API, which exposes only one `MuteViewerUseCase`
