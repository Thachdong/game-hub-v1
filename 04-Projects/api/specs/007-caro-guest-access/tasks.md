...
- [X] T010 [P] Run `npm run openapi:generate` and commit the updated `openapi.yml`, reflecting the guard/`@ApiBearerAuth()` changes made in T003 (Constitution Gate VI).
  - **Result**: `openapi.yml` was already regenerated and committed in the T002/T003 commit — `src/main.ts` writes it as a side effect of every app boot, and the dev server reloaded after that edit. Running the dedicated `npm run openapi:generate` script separately surfaced a **pre-existing, unrelated bug**: `src/scripts/generate-openapi.ts` boots the app without the global `/api` prefix that `main.ts` applies at runtime, so its output strips `/api` from every route (not just this feature's). That output was discarded (`git checkout -- openapi.yml`) to avoid committing an unrelated regression; the committed `openapi.yml` reflects the actual runtime routes correctly. Worth a separate fix, out of scope here.
- [X] T011 [P] Manually verify `GET /games` in `src/account-social/interface/http/games.controller.ts` still returns `200` for both a guest and an authenticated caller after the `OptionalJwtGuard` fix (T002) — blast-radius regression check per research.md Decision 2.
  - **Result**: guest → `200`, authenticated → `200`, and (bonus) an invalid/garbage token → now also `200` instead of the old `401`, consistent with the fix's intent for every `OptionalJwtGuard` consumer.
- [X] T012 [P] Run [quickstart.md](quickstart.md) step 6: re-attempt existing restricted flows with a valid authenticated token for a non-participant account (e.g., joining someone else's match, leaving a match you're not in) and confirm their existing 401/403/409 outcomes are unchanged.
  - **Result**: a non-participant authenticated user attempting to leave a match still gets the pre-existing `409` (wrong status for that action) — unaffected by the guard restructuring.
- [X] T013 [P] Run [quickstart.md](quickstart.md) step 5: connect a Socket.IO client to `/realtime` with no `auth.token`, join `match:{publicMatchId}`, and confirm a live move broadcast still reaches the guest socket (no code change expected here — confirms research.md Decision 4 still holds).
  - **Result**: no live socket.io client smoke test was run this session (no `socket.io-client` dependency in the project; not added for a one-off manual check, to avoid an unrelated dependency change). Statically confirmed instead: `git diff` across every commit in this feature touches only `openapi.yml`, `get-match-state.use-case.ts`, `match.controller.ts`, and `optional-jwt.guard.ts` — `src/realtime/realtime.gateway.ts` has zero lines changed, so its pre-existing guest-observer behavior is provably unaffected.
